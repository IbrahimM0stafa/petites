...
export const authInterceptor: HttpInterceptorFn = (request, next) => {
	...
	const skipGuestHeader = request.context.get(SKIP_GUEST_HEADER);
	const accessToken = authState.accessToken();
	const guestToken = authState.guestSessionToken();
	...
	} else if (!skipGuestHeader && !accessToken && guestToken) {
		outgoingRequest = outgoingRequest.clone({ setHeaders: { 'X-Guest-Token': guestToken } });
	}
